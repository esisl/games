...
function sprite(id, filename, width, height, count){
	...
	this.pwidth=width;//если потребуется изменит...
	this.pheight=height;
	...
	//if(mirror){s+="-moz-transform: scaleX(-1); -webkit-transform: scaleX(-1); -o-transform: scaleX(-1); transform: scaleX(-1); -ms-filter: fliph; filter: fliph;";}
		s+='"></div>';
		//debug.innerHTML+='frameX='+frameX+', frameY='+frameY+', w='+this.width+', h='+this.height+'<br>';
		if(ctx!=null){
			ctx.drawImage(this.img, frameX, frameY, this.width, this.height, (x*1+this.alignX), (y*1+this.alignY), this.pwidth, this.pheight);
		}
